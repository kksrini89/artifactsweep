...
pub fn find_junk_paths(path: &Path) -> Vec<PathBuf> {
    let mut found = vec![];
...
pub fn size_entries(paths: &[PathBuf]) -> Vec<JunkEntry> {
    if paths.is_empty() {
        return Vec::new();
    }
    ...
    // due to calculating size operation done irregular orders
    entries.sort_by(|a, b| b.size_bytes.cmp(&a.size_bytes));

    entries
}

pub fn print_junk_report(paths: &[JunkEntry]) {
    if paths.is_empty() {
